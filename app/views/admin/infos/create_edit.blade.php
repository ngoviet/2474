...
@section('content')
	...
	<!-- ./ tabs -->
	 ...
	 Edit Blog Form
	<form class="form-horizontal" method="post" action="@if (isset($post)){{ URL::to('admin/infos/' . $post->id . '/edit') }}@endif" autocomplete="off">
		<!-- CSRF Token -->
...
<!--                        <textarea class="textarea wysihtml5-editor">{{{ Input::old('content', isset($post) ? $post->content : null) }}}</textarea>-->
<!--                        <div class="form-control" id="summernote" name="content" value="content">{{{ Input::old('content', isset($post) ? $post->content : null) }}}"</div>-->
<!--                        <input id="summernote" name="content" value="{{{ Input::old('content', isset($post) ? $post->content : null) }}}"/>-->
		...
		<div class="form-group">
			<div class="col-md-12">
				<element class="btn-info btn close_popup">Cancel</element>
